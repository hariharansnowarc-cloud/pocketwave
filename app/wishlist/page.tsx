...
export default function WishlistPage() {
  ...
  async function addWish() {
    ...
    setSaving(true)
    await supabase.from('wishes').insert({ title: form.title.trim(), description: form.description.trim() || null, tentative_date: form.tentative_date.trim() || null, added_by: form.added_by, status: 'active' })
  ...
  async function updateStatus(id: string, status: 'completed' | 'cancelled') {
    await supabase.from('wishes').update({ status, completed_at: status === 'completed' ? new Date().toISOString() : null, cancelled_at: status === 'cancelled' ? new Date().toISOString() : null }).eq('id', id)
    loadWishes()
  }
  ...
  return (
    <main style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      <Nav />
      <section style={{ maxWidth: '680px', margin: '0 auto', padding: '4rem 2rem 4rem' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '2rem' }}>
          <div>
            ...
            <h1 className="fade-up" style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '3rem', fontWeight: 600, color: 'var(--ink)', lineHeight: 1.1 }}>Wishlist ✨</h1>
          </div>
          <button onClick={() => setShowForm(!showForm)} className="btn-hover"
        ...
        {showForm && (
          <div className="fade-up" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ marginBottom: '10px' }}>
              ...
              <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. Trip to Ooty, Dinner at..." style={inp} />
            </div>
            <div style={{ marginBottom: '10px' }}>
              <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--muted)', display: 'block', marginBottom: '5px' }}>Description</label>
              <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="More details..." rows={2} style={{ ...inp, resize: 'none', lineHeight: 1.6 }} />
            </div>
            <div style={{ marginBottom: '1.25rem' }}>
              <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--muted)', display: 'block', marginBottom: '5px' }}>Tentative date / time</label>
              <input value={form.tentative_date} onChange={e => setForm({ ...form, tentative_date: e.target.value })} placeholder="e.g. June 2026, After marriage..." style={inp} />
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={addWish} disabled={saving || !form.title.trim() || !form.added_by} className="btn-hover"
        ...
        {tab === 'active' && (
          loading ? <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--subtle)' }}>Loading...</div>
          : active.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '5rem 2rem', color: 'var(--subtle)' }}>
              ...
              <button onClick={() => setShowForm(true)} className="btn-hover" style={{ background: 'var(--rose)', color: 'white', border: 'none', padding: '10px 24px', borderRadius: '999px', fontSize: '14px', fontWeight: 500, cursor: 'pointer' }}>+ Add wish</button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {active.map(wish => (
                <div key={wish.id} className="card-hover" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '14px', padding: '1.25rem' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px', marginBottom: '8px' }}>
                    <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '19px', fontWeight: 600, color: 'var(--ink)' }}>{wish.title}</h3>
                    <button onClick={() => deleteWish(wish.id)} style={{ fontSize: '13px', color: 'var(--subtle)', background: 'none', border: 'none', cursor: 'pointer', flexShrink: 0, padding: '0 4px' }}>×</button>
                  </div>
                  {wish.description && <p style={{ fontSize: '14px', color: 'var(--muted)', lineHeight: 1.6, marginBottom: '10px' }}>{wish.description}</p>}
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '12px' }}>
                    {wish.tentative_date && <span style={{ fontSize: '11px', color: 'var(--gold)', background: 'var(--gold-light)', padding: '3px 10px', borderRadius: '999px', fontWeight: 500 }}>📅 {wish.tentative_date}</span>}
                    <span style={{ fontSize: '11px', color: 'var(--rose)', background: 'var(--rose-light)', padding: '3px 10px', borderRadius: '999px', fontWeight: 500 }}>Added by {wish.added_by}</span>
                    <span style={{ fontSize: '11px', color: 'var(--subtle)', padding: '3px 0' }}>{new Date(wish.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => updateStatus(wish.id, 'completed')} className="btn-hover" style={{ fontSize: '12px', fontWeight: 500, color: 'var(--green)', background: 'var(--green-light)', border: '1px solid var(--green)', padding: '6px 14px', borderRadius: '999px', cursor: 'pointer' }}>✓ Mark done</button>
                    <button onClick={() => updateStatus(wish.id, 'cancelled')} className="btn-hover" style={{ fontSize: '12px', fontWeight: 500, color: 'var(--muted)', background: 'var(--cream2)', border: '1px solid var(--border2)', padding: '6px 14px', borderRadius: '999px', cursor: 'pointer' }}>✕ Cancel</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab === 'history' && (
          history.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {history.map(wish => (
                <div key={wish.id} className="card-hover" style={{ background: '#fff', border: `1px solid ${wish.status === 'completed' ? 'var(--green)' : 'var(--border)'}`, borderRadius: '14px', padding: '1.25rem', opacity: 0.85 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px', marginBottom: '8px' }}>
                    ...
                    <button onClick={() => deleteWish(wish.id)} style={{ fontSize: '13px', color: 'var(--subtle)', background: 'none', border: 'none', cursor: 'pointer', flexShrink: 0 }}>×</button>
                  </div>
                  {wish.description && <p style={{ fontSize: '13px', color: 'var(--muted)', lineHeight: 1.6, marginBottom: '8px' }}>{wish.description}</p>}
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
                    ...
                    {wish.tentative_date && <span style={{ fontSize: '11px', color: 'var(--gold)', background: 'var(--gold-light)', padding: '3px 10px', borderRadius: '999px' }}>📅 {wish.tentative_date}</span>}
                    <span style={{ fontSize: '11px', color: 'var(--rose)', background: 'var(--rose-light)', padding: '3px 10px', borderRadius: '999px' }}>By {wish.added_by}</span>
                  </div>
                  <button onClick={() => restoreWish(wish.id)} className="btn-hover" style={{ fontSize: '12px', color: 'var(--muted)', background: 'var(--cream2)', border: '1px solid var(--border2)', padding: '5px 14px', borderRadius: '999px', cursor: 'pointer' }}>↩ Restore</button>
                </div>
              ))}
            </div>
          )
