...
export default function GalleryPage() {
  ...
  const inp: React.CSSProperties = {
    ...
    padding: '10px 14px', fontSize: '14px', color: 'var(--ink)', outline: 'none',
  ...
  return (
    <main style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <section style={{ maxWidth: '960px', margin: '0 auto', padding: '3rem 1.25rem 6rem' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.25rem', marginBottom: '1.5rem' }}>
            ...
            {addingCategory && (
              <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                ...
                <button onClick={addCategory}
                  style={{ background: 'var(--rose)', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: 500, whiteSpace: 'nowrap' }}>
                  Add
                </button>
              </div>
            )}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {categories.map(c => (
                <button key={c} onClick={() => setCategory(c)}
                  style={{ padding: '5px 12px', borderRadius: '999px', fontSize: '12px', fontWeight: 500, border: `1px solid ${category === c ? 'var(--rose)' : 'var(--border2)'}`, background: category === c ? 'var(--rose-light)' : '#fff', color: category === c ? 'var(--rose)' : 'var(--muted)', cursor: 'pointer' }}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <input type="text" value={caption} onChange={e => setCaption(e.target.value)}
              placeholder="Add a caption..." style={inp} />
          </div>
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
          {['All', ...categories].map(c => (
            <button key={c} onClick={() => setActiveCategory(c)}
              style={{ padding: '6px 14px', borderRadius: '999px', fontSize: '12px', fontWeight: 500, border: `1px solid ${activeCategory === c ? 'var(--rose)' : 'var(--border2)'}`, background: activeCategory === c ? 'var(--rose)' : '#fff', color: activeCategory === c ? 'white' : 'var(--muted)', cursor: 'pointer' }}>
        ...
        {loading ? (
            ...
            {filtered.map(photo => (
              <div key={photo.id} onClick={() => setSelected(photo)}
                ...
                <div style={{ padding: '8px 10px' }}>
                  ...
                  <p style={{ fontSize: '10px', color: 'var(--subtle)', marginTop: '3px' }}>
                    {photo.uploaded_by} · {new Date(photo.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div style={{ textAlign: 'center', padding: '4rem 1rem', color: 'var(--subtle)' }}>
            ...
            <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '20px', color: 'var(--ink)', marginBottom: '6px' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <input value={commentBody} onChange={e => setCommentBody(e.target.value)}
                    ...
                    style={{ flex: 1, background: '#fff', border: '1px solid var(--border2)', borderRadius: '8px', padding: '8px 10px', fontSize: '13px', color: 'var(--ink)', outline: 'none' }} />
