...
export default function TimelinePage() {
  ...
  return (
    <main style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <section style={{ maxWidth: '640px', margin: '0 auto', padding: '4rem 2rem 6rem' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '3rem' }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '3rem', fontWeight: 600, color: 'var(--ink)', lineHeight: 1.1 }}>How it all began</h1>
          </div>
        ...
        {showForm && (
          <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.5rem', marginBottom: '2.5rem' }}>
            <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '20px', fontWeight: 600, color: 'var(--ink)', marginBottom: '1.25rem' }}>
              {editingId ? 'Edit moment' : 'Add a new moment'}
            </h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '64px 1fr', gap: '10px', marginBottom: '10px' }}>
              <div>
                ...
                <input value={form.emoji} onChange={e => setForm({ ...form, emoji: e.target.value })}
                  style={{ ...inp, textAlign: 'center', fontSize: '20px', padding: '10px 8px' }} maxLength={2} />
              </div>
              <div>
                <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--muted)', display: 'block', marginBottom: '5px' }}>Title *</label>
                <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
                  placeholder="e.g. We met for the first time" style={inp} />
              </div>
            </div>
            ...
            <div style={{ marginBottom: '10px' }}>
              ...
              <input value={form.event_date} onChange={e => setForm({ ...form, event_date: e.target.value })}
                placeholder="e.g. March 2024 or 14 Feb 2025" style={inp} />
            </div>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                placeholder="Tell the story of this moment..." rows={3}
                style={{ ...inp, resize: 'none', lineHeight: 1.6 }} />
            </div>

            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={save} disabled={saving || !form.title.trim() || !form.event_date.trim() || !form.added_by}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              {events.map((event) => (
                <div key={event.id} style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'var(--rose-light)', border: '2px solid var(--rose-mid)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0, position: 'relative', zIndex: 1 }}>
                    {event.emoji}
                  </div>
                  <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '14px', padding: '1.25rem', flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px', marginBottom: '6px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                        <button onClick={() => startEdit(event)}
                          style={{ fontSize: '12px', color: 'var(--muted)', background: 'var(--cream2)', border: 'none', padding: '4px 10px', borderRadius: '999px', cursor: 'pointer' }}>
                          Edit
                        </button>
                        <button onClick={() => deleteEvent(event.id)} disabled={deletingId === event.id}
                          style={{ fontSize: '12px', color: '#dc2626', background: '#fef2f2', border: 'none', padding: '4px 10px', borderRadius: '999px', cursor: 'pointer', opacity: deletingId === event.id ? 0.5 : 1 }}>
                    ...
                    {event.description && (
                      <p style={{ fontSize: '14px', color: 'var(--muted)', lineHeight: 1.7, marginBottom: '10px' }}>{event.description}</p>
                    )}
